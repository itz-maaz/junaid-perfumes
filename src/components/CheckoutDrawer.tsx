      ...
      >
        ...
        <form onSubmit={handlePay} className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
              ...
              {items.map((it) => (
                <li key={it.id} className="flex items-center justify-between gap-3 text-sm rounded-lg border border-white/5 bg-zinc-950/40 p-2.5">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    <img src={it.image} alt={it.name} className="h-12 w-10 rounded object-cover" />
          ...
          <div className="pt-2 space-y-3">
            <button
              type="submit"
              className="w-full rounded-full bg-brand-green py-3.5 text-sm font-semibold uppercase tracking-[0.2em] text-brand-green-foreground shadow-lg shadow-brand-green/20 transition active:scale-[0.98] inline-flex items-center justify-center gap-2"
            >
